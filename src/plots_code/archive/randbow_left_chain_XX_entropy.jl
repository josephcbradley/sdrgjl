# How does negativity scale with randbow? 
# Do 2x2 with δ = 1.0 and h = (0.5, 3.0, 7.0, 10.0)
hs = (BigFloat.([0.5, 3., 7., 10.]))
negativity_randbow_plot_vec = Plots.Plot{Plots.PGFPlotsXBackend}[]
trials = 2_000
for h in reverse(hs)
    println("Running entanglement negativity plot for r = 0 with h = $h, δ = 1.0")
    randbow = RandbowDistribution(BigFloat(1.0), BigFloat(h))
    randbow_params = SDRGParams(L = 1000, boundaries = :open, Δ = 0.0)
    randbow_results_1000 = disorder_negativity_average(randbow_params, trials, randbow, 0,
    ...
    randbow_params = SDRGParams(L = 2000, boundaries = :open, Δ = 0.0)
    randbow_results_2000 = disorder_negativity_average(randbow_params, trials, randbow, 0,
        start_pos = 1, min_window_size = 10, window_increment = 10,
        L = randbow_params.L)
...
fixed_d_vary_h_negativity_delta_0 = plot(negativity_randbow_plot_vec..., layout = (2, 2), dpi = 600)

savefig(fixed_d_vary_h_negativity_delta_0, img_path * "randbow_left_chain_XX_entropy" * ".png")